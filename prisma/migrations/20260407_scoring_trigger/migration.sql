...
-- Replaces src/lib/analysis/daily-score.ts
-- Computes computedMood and computedScore on entry insert/update
-- ============================================================

-- ──────────────────────────────────────────────
-- Main scoring function
-- Called by trigger on entries table
-- ──────────────────────────────────────────────
...
BEGIN
  -- Parse input from the entry row
  ...
  -- If no behaviors checked, return neutral (no computation needed)
  ...
  -- Find the active framework for this tenant
  ...
  -- ──────────────────────────────────────────────
  -- Build criteria sets per pole
  -- Uses a temp table to track unique criteria per pole
  -- ──────────────────────────────────────────────
  ...
  -- ──────────────────────────────────────────────
  -- Process behavior keys → criterion mappings
  -- ──────────────────────────────────────────────

  -- For each behavior key, look up its criterion mappings
  FOR rec IN
    SELECT
      bd."itemKey",
      bd."isSafetyConcern",
  ...
  LOOP
    ...
    -- Add criterion to set (UNIQUE constraint prevents double-counting)
  ...
  LOOP
    ...
    IF rec.adds_criterion_number IS NOT NULL AND rec.adds_criterion_pole_slug IS NOT NULL THEN
      ...
      -- Check if the added criterion is CORE type
  ...
  -- ──────────────────────────────────────────────
  -- Check if any accumulated criteria are CORE type
  -- (a behavior might add a criterion that happens to be CORE
  --  even though the behavior's own mapping type was STANDARD)
  -- ──────────────────────────────────────────────
  ...
  -- ──────────────────────────────────────────────
  -- Safety concern from impairments
  -- ──────────────────────────────────────────────
  ...
  -- ──────────────────────────────────────────────
  -- Apply classification rules (priority descending)
  -- First match wins
  -- ──────────────────────────────────────────────
  ...
  LOOP
    -- Get criteria count for this rule's pole
    SELECT COUNT(*) INTO v_pole_count
    FROM _criteria_sets
    WHERE pole_slug = rule_rec.pole_slug;

    -- Get gate/core status for this pole
    ...
    -- Threshold with gate-only adjustment
    ...
    -- Check criteria count meets threshold
    IF v_pole_count >= v_threshold THEN
      -- Mixed features check
  ...
  IF NOT v_matched THEN
    ...
    LOOP
      ...
      -- Gate check for subthreshold
      IF rule_rec."gateRequired" AND NOT COALESCE(rec.gate_met, FALSE) THEN
        CONTINUE;
      END IF;

      -- Core check for subthreshold
      IF rule_rec."coreRequired" AND NOT COALESCE(rec.core_met, FALSE) THEN
        CONTINUE;
      END IF;
  ...
  -- ──────────────────────────────────────────────
  -- Set computed fields on the entry
  -- ──────────────────────────────────────────────
...
-- ──────────────────────────────────────────────
-- Trigger: fire on insert or update of entries
-- ──────────────────────────────────────────────

DROP TRIGGER IF EXISTS trg_compute_daily_score ON entries;

CREATE TRIGGER trg_compute_daily_score
  BEFORE INSERT OR UPDATE ON entries
  FOR EACH ROW
  EXECUTE FUNCTION compute_daily_score();

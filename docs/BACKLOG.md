...
- [M0-2] **[DONE 2026-07-09]** (60m) Web unit-test infra — Vitest + @testing-library/react + jsdom in `web/`; `npm test` script; first real test: `Reports.extractScore`. **AC met:** 3 tests green (RPT-1/3/4 slices); build still green.
- [M0-3] **[DONE 2026-07-09]** (60m) Mobile unit-test infra — jest-expo + @testing-library/react-native; declared `react-native-paper` + `@expo/vector-icons` (F12 closed); `generateUUID` tests. **AC met:** 2 tests green.
- [M0-4] **[DONE 2026-07-09]** (90m) Framework seed recovery — `db/seed/dsm5-bipolar.sql`, generated mechanically from the recovered script (reference copy in `db/seed/reference/`); idempotent natural-key upserts. **AC met:** fresh DB + migrations + seed → SC-1 insert computes MANIC. CLINICAL-REVIEW: values copied verbatim, zero retyping; Maria may diff against the reference file.
- [M0-5] **[DONE 2026-07-09]** (90m) Postgres test rig — `db/rig/` (shim, setup, ST-004-mirror grants, F23 compat overlay) + `db/tests/` Vitest suite. **AC met:** 11 green + ISO-9 pinned expected-fail (F4 proven). Discovered F23: committed analysis migration cannot compile — see QUESTIONS.
...
- [M0-7] **[DONE 2026-07-09]** (45m) Scoring-suite completion — 18 SC + 8 EP + 4 ISO cases; 28 green, 2 expected-fail pins. **Found F24/F25/F26** (subthreshold-MIXED fallback dead, gap semantics, daily-vs-episode DSM disagreement) — see QUESTIONS; fixes routed to M5-3 pending D-6.
- [M0-8] **[DONE 2026-07-09 — verify on first staging push]** (45m) GitHub Actions CI — `.github/workflows/tests.yml`: web (test+build), mobile (jest), db (postgres:16 service + rig) on push/PR to staging/main. CI database path verified locally via `RIG_SUPERUSER_URL`; the live green run can only be confirmed after this lands on `staging`. *(Approved D-5.)*

## M1 — Phase A: core stability
- [M1-1] **[DONE 2026-07-09 — ST-077 on-stage, device verification pending]** (90m) **ST-077** ProjectProvider resilience — auth-ready gate, error state + one auto-retry + shared Retry banner in the tabs layout, `getJwt()` null no longer signs out (F18). **AC met:** MOB-1/1b/2/3/4/4b/8 green (mobile suite 10/10); cold-start device check is DEVICE-GATED.
...
- [M5-3] (—) Scoring-defect fixes bundle — F8 (withdrawal-trend signal: restore with Maria's threshold review, or de-document), F9 (episodes read persisted counts), F7 (remove temp-table coupling; framework-agnostic poles), F24 (subthreshold-MIXED fallback dead code + nondeterministic ties), F25 (gap semantics), F26 (gateOnlyAdjustment + episode DSM-day check consistency — unless D-6 pulls it earlier). **CLINICAL-REVIEW.** *(D-4 answered 2026-07-09: split — F6 moved to M1-1b.)* Tests first (SIG-8, PRD-7, SC-8 flip from pinned to green).

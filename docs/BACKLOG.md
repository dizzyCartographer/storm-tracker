...
- [M0-5] (90m) Postgres test rig — per build-spec §5.1: dockerized Postgres 16, migration applier, `auth.user_id()` shim, role switching, Vitest+pg runner in `db/tests/`. **AC:** SC-1, SC-2, SC-16, ISO-1, ISO-2 implemented and passing (or failing with findings filed). BLOCKED(D-3) until Maria approves the rig approach.
...
- [M1-10] (60m) Authz/function repair migration — fix `accept_invite` (use `used` boolean, correct tenant_members columns), fix `create_tenant_with_owner` columns, add RLS to `jwks`, add owner-only UPDATE policy to `tenant_members`. **AC:** INV-2..5, ISO-9, ISO-10 green in rig. **BLOCKED(D-2)** — schema/authz change needs Maria's approval; also depends on M0-6 (live DBs may already differ).
- [M1-11] (45m) Fix `web/api/invite-details.ts` — correct table names, env-driven URL (F22), minimal field exposure; keep public-by-design behavior. **AC:** INV-5 green; invite landing works on staging. BLOCKED(D-2).
...
- M0-5 rig approach → **D-3** · M1-10/M1-11 invite repair → **D-2** · M5-3 scoring defects → **D-4** · CI → **D-5** · the harness itself → **D-1**.
